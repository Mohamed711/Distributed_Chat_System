        ...
        try 
        {
            ...
            while (true) {
                ...
                switch (header) {
                    case "sign_in":
                        ...
                        if(GlobalVariables.clients.containsKey(name))
                        {
                            ...
                            else
                            {
                                ...
                                mainGui.updateUserStatus(true, name);
                            }
                        }
                        else
                        {
                    ...
                    case "unenroll_grp":
                        {
                            ...
                            Group trmvGrp = GlobalVariables.groups.get(grpName);
                            trmvGrp.removeClient(this.currentClient);
                            if(trmvGrp.getClients().size()==0)
                            {
                                GlobalVariables.groups.remove(grpName);
                                gui.updateAllUsers_deleteGroup(grpName);
                            }
                            break;
                        }
                    case "grp_msg":
                    ...
                    {
                        Client cValue = entry.getValue();
                        if(entry.getKey()!=currentClient.getName())
                        {
                            s.append(cValue.getName()+",");
                        }
                    }       sOut = s.toString();
                            sOut = sOut.substring(0, sOut.length()-1);
                            sOut = currentClient.getName()+","+sOut;
                    ...
                    case "sign_out":
                        ...
                        mainGui.updateUserStatus(false, this.currentClient.getName());
                        break OUTER;
                }
            }
            //release resource
